...
- Текст берется из поля `commentText` выбранного SMS-шаблона (прямая привязка комментария к шаблону).
- Если у активного шаблона комментарий не заполнен, создается ошибка `COMMENT_TEXT_RESOLVE_FAILED`.
